...
5. 如需 BT/磁力搜索，在“转存配置 → 资源搜索源”填写 Prowlarr（推荐）或 Jackett 的地址和 API Key；结果会统一进入资源详情页并可提交 115 离线下载。
6. “观影/盘搜”只接受你有权使用的公开、稳定 JSON 搜索 API；未配置时保持关闭，不会使用页面抓取或绕过验证码。
7. 如需通知，在 Telegram 页面配置 Bot Token、Chat ID 和事件模板。
8. 启用频道监控后，首次检查只建立消息游标，不处理历史消息，避免意外消耗积分。
