...
async function subscribeToggle(x,b){if(!b)return;b.disabled=true;try{const r=await fetch('/api/web/subscriptions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:x.title,media_type:x.media_type,tmdb_id:x.tmdb_id||0,douban_id:x.douban_id||'',year:x.year?+x.year:null,poster:x.poster||''})}),d=await r.json();if(!r.ok)throw Error(d.detail||'订阅失败');b.classList.add('subscribed');b.title='已订阅';b.setAttribute('aria-pressed','true')}catch(e){alert(e.message)}finally{b.disabled=false}}
